...
public class MilcomListenerImpl {



    public static void milcomListener(MessageCreateEvent messageCreateEvent, WarroomRepository warRoomDao, UserRepository userDao) {
        ...
        String hkRole = Config.mainServerHkRole;
            ...
            if (Arrays.asList(Config.warCategories).contains(messageCreateEvent.getServerTextChannel().get().getCategory().get().getIdAsString()) && messageCreateEvent.getServer().get().getIdAsString().equalsIgnoreCase(serverId)) {
...
//                        Role everyoneRole = server.get().getEveryoneRole();
                            ...
                            ChannelCategory category = api.getChannelCategoryById(warCategory).get();
                            boolean full = category.getChannels().size() == 50;
                            while (full) {
                                for (String id : Config.warCategories) {
                                    category = api.getChannelCategoryById(id).get();
